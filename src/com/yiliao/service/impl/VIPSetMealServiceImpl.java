...
public class VIPSetMealServiceImpl extends ICommServiceImpl implements
	...
	public JSONObject getVipSetMealList(int page) {
		...
		try {
			...
			String sql = "SELECT t_id,t_setmeal_name,t_cost_price,t_money,t_duration,t_pay_type,t_set_meal_name,t_is_enable,DATE_FORMAT(t_create_time,'%Y-%m-%d %T') AS t_create_time  FROM t_vip_setmeal  order by t_id desc   LIMIT ?,10";
	...
	public MessageUtil saveVipSetMeal(Integer t_id, String t_setmeal_name,
			...
			int t_is_enable,int t_gold,int t_payType,String t_setMealName) {
		try {
			//新增
			if(null == t_id ||  0 == t_id){
				String inSql = "INSERT INTO t_vip_setmeal (t_setmeal_name, t_cost_price, t_money, t_duration, t_is_enable,t_gold,t_pay_type,t_set_meal_name, t_create_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
				this.getFinalDao().getIEntitySQLDAO().executeSQL(inSql, t_setmeal_name,t_cost_price,t_money,t_duration,t_is_enable,t_gold,t_payType,t_setMealName,DateUtils.format(new Date(), DateUtils.FullDatePattern));
			}else{ //修改
				String upSql = "UPDATE t_vip_setmeal SET   t_setmeal_name=?, t_cost_price=?, t_money=?, t_duration=?, t_is_enable=?, t_gold = ?,t_pay_type = ? ,t_set_meal_name = ? WHERE t_id=?;";
				this.getFinalDao().getIEntitySQLDAO().executeSQL(upSql, t_setmeal_name,t_cost_price,t_money,t_duration,t_is_enable,t_gold,t_payType,t_setMealName,t_id);
			}
