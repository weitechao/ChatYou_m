...
public class CashValueSetMealServiceImpl extends ICommServiceImpl implements
	...
	public JSONObject getCashValueList(int t_project_type,int t_end_type,int page) {
		...
		try {
			...
			String sql = "SELECT t_id,t_project_type,t_gold,t_money,t_is_enable,t_end_type,t_describe,t_set_meal_name,DATE_FORMAT(t_create_time,'%Y-%m-%d %T') AS t_create_time  FROM t_set_meal WHERE 1=1 ";
	...
	public MessageUtil addOrUpdateCashValue(Integer t_id, int t_project_type,
			int t_gold, BigDecimal t_money, int t_end_type,int t_is_enable,String t_describe,String t_setMelName) {
		try {
			//新增
			if(null == t_id ||  0 == t_id){
				String inseSql = "INSERT INTO t_set_meal (t_project_type, t_gold, t_money,t_end_type,t_describe,t_is_enable,t_set_meal_name,t_create_time) VALUES (?,?,?,?,?,?,?,?);";
				
				this.getFinalDao().getIEntitySQLDAO().executeSQL(inseSql, t_project_type,t_gold,t_money,t_end_type,t_describe,t_is_enable,t_setMelName,
						DateUtils.format(new Date(), DateUtils.FullDatePattern));
				
			}else{ //修改
				
				String upSql = "UPDATE t_set_meal SET  t_project_type=?, t_gold=?, t_money=?,t_end_type = ?, t_is_enable=?, t_describe = ?,t_set_meal_name= ?  WHERE t_id=?";
				
				this.getFinalDao().getIEntitySQLDAO().executeSQL(upSql, t_project_type,t_gold,t_money,t_end_type,t_is_enable,t_describe,t_setMelName,t_id);
