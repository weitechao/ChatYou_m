...
public class SystemSetUpServiceImpl extends ICommServiceImpl implements SystemSetUpService {
	...
	public MessageUtil setSystemSetUp(int t_id, int t_scope, String t_android_download, String t_ios_download,
			...
			String t_nickname_filter,String t_video_hint,String t_spreed_hint,int t_zhekou,int t_zuidi,String t_parameter) {

		try {

			String uSql = " UPDATE t_system_setup SET t_scope = ? ,t_android_download = ? ,t_ios_download = ?, t_system_lang_girl = ?, t_system_lang_male = ?,t_default_text = ? ,t_default_video = ?,t_default_phone = ?,t_default_weixin = ?,t_award_rules = ?,t_service_qq = ? ,t_nickname_filter = ?,t_video_hint = ? ,t_spreed_hint = ?  ,t_zhekou = ?,t_zuidi = ?,t_parameter = ?  WHERE t_id = ?  ";
			...
			this.getFinalDao().getIEntitySQLDAO().executeSQL(uSql, t_scope, t_android_download, t_ios_download,
					...
					t_default_weixin, t_award_rules, t_service_qq, t_nickname_filter,t_video_hint,t_spreed_hint, t_zhekou,t_zuidi, t_parameter, t_id);
